...
angular.module('serveApp')
  .controller('MainCtrl', function ($scope,$state,$stateParams,$cookieStore, $cookies, $window,$location,RestWecom,RestWechat) {
    ...
    if($stateParams.vecter){
      $cookies.put('vecter', $stateParams.vecter); 
    }
    if($scope.wxUser){
  ...
  .controller('SwiperCtrl',function($scope, $rootScope, $stateParams, $cookies, Restangular, $uibModal, Merchant, appConfig, RestDocument){
    $scope.swiper = {};
    ...
    Restangular.one('agent').one('prp').customGET("number", {alias: $scope.vecter})
      .then(res => {
        $scope.data = res;
      })
  })
