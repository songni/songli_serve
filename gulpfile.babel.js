...
let styles = lazypipe()
    ...
    .pipe(plugins.sass)
    .pipe(plugins.autoprefixer, {browsers: ['> 5% in CN']})
...
gulp.task('watch', () => {
    ...
    plugins.livereload.listen({port:35711});
    ...
    plugins.watch(paths.client.styles, () => {  //['inject:sass']
        gulp.src(paths.client.mainStyle)
            ...
            .pipe(styles())
            .pipe(gulp.dest('.tmp/app'))
            .pipe(plugins.livereload({port:35711}));
    });

    plugins.watch(paths.client.views)
        .pipe(plugins.plumber())
        .pipe(plugins.livereload({port:35711}));
    // 10ms so faster for sftp file readDelay set to 100 ms
    ...
    plugins.watch(,() =>{
      gulp.src('./*')
        .pipe(plugins.plumber())
        .pipe(plugins.git.add({args: '-f -i -p'}))
        .pipe(plugins.git.commit('auto commit'))
        .on('data',function(data) {
          console.log(plugins.util.colors.white('[') +
              plugins.util.colors.yellow('git:commit') +
              plugins.util.colors.white('] ') +
              data);
        })
        .pipe(plugins.git.push());
    });*/
    //var watcher = gulp.watch(_.union(paths.client.scripts, [`!${clientPath}/**/*.{spec,mock}.js`, `!${clientPath}/app/app.js`]), ['git:push']);
...
//         .pipe(plugins.plumber())
//         .pipe(plugins.git.add({cwd:'./',args: '-f -i -p'}))
//         .on('data',function(data) {
//           console.log(plugins.util.colors.white('[') +
//               plugins.util.colors.yellow('git:commit') +
//               plugins.util.colors.white('] '),
...
//     var stream = gulp.src('./*')
//         .pipe(plugins.plumber())
//         .pipe(plugins.git.commit(""+timestamp+"", {cwd:'./*'}));
//     return stream
// });
...
gulp.task('html', function() {
    return gulp.src(`${clientPath}/{app,components}/**/*.html`)
      ...
      .pipe(plugins.angularTemplatecache({module: 'serveApp'}))
      .pipe(gulp.dest('.tmp'));
});

gulp.task('constant', function() {
  let sharedConfig = require(`./${serverPath}/config/environment/shared`);
  let appConfig = sharedConfig[process.env.NODE_ENV];
  return plugins.ngConstant({
    name: 'serveApp.constants',
    deps: [],
    wrap: true,
    stream: true,
...
gulp.task('copy:media', () => {
    return gulp.src(`${clientPath}/bower_components/zeroclipboard/dist/ZeroClipboard.swf`, { dot: true })
        .pipe(gulp.dest(`${paths.dist}/${clientPath}/bower_components/zeroclipboard/dist/ZeroClipboard.swf`));
});
gulp.task('copy:tinymce-dist', () => {
    return gulp.src(`${clientPath}/bower_components/tinymce-dist/{plugins,skins,themes}/**/**/*`, { dot: true })
...
gulp.task('cdn',function(){
  return gulp.src(`${paths.dist}/${paths.client.mainView}`)
    .pipe(plugins.cdnizer({
        defaultCDNBase: "//img.91pintuan.com/serve",
        allowRev: true,
        allowMin: false,
        files: [
            'app/*.js',
